...
class TribeRunner:
    ...
    def run(self, input_path: str | Path, *, verbose: bool = True, save_to: str | Path | None = None) -> PredictionResult:
    ...
    def get_brain_stimulus(self, result_or_input: PredictionResult | str | Path, *, verbose: bool = True):
        # Accept either a ready result or a raw input path and return the array payload.
        result = self.workflow.resolve_prediction_result(result_or_input, self.run, verbose)
        return result.brain_stimulus

    def get_brain_stimulus_dataframe(self, result_or_input: PredictionResult | str | Path, *, verbose: bool = True) -> pd.DataFrame:
        # Accept either a ready result or a raw input path and return a tabular view.
        result = self.workflow.resolve_prediction_result(result_or_input, self.run, verbose)
        return result.brain_stimulus_frame()

    def save_output(self, result: PredictionResult, output_path: str | Path | None = None, *, include_brain_stimulus_csv: bool = False) -> Path:
